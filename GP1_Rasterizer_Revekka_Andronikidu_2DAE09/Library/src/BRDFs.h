...
namespace dae
{
	namespace BRDF
	{
		...
		static ColorRGB Phong(float ks, float exp, const Vector3& l, const Vector3& v, const Vector3& n)
		{
			...
			float cosA = Vector3::DotClamp(reflect, v);
		...
		static float NormalDistribution_GGX(const Vector3& n, const Vector3& h, float roughness)
		{
			...
			float GGX = Square(alpha) / (PI * Square(Square(Vector3::DotClamp(n, h)) * (Square(alpha) - 1) + 1));
			return GGX;
		}
		...
		{
			float alpha = roughness * roughness;
			float k = ((alpha + 1) * (alpha + 1)) / 8.f;
			return { Vector3::DotClamp(n,v) / (Vector3::DotClamp(n,v) * (1.f - k) + k) };
		}
